...
# Svuota la cache della GPU per liberare memoria prima di caricare un nuovo modello
# Questo è utile in caso di OutOfMemoryError.
if torch.cuda.is_available():
...
# Usiamo `device_map={"": 0}` per forzare il caricamento dell'intero modello sulla GPU 0.
# Questo può essere necessario se "auto" tenta di allocare su CPU/disco per moduli incompatibili con la quantizzazione,
# ma in realtà si intende che tutto il modello quantizzato vada sulla GPU.
model_new = AutoModelForCausalLM.from_pretrained(
    model_name_new,
    quantization_config=bnb_config,
    device_map={"": 0} # Impostato esplicitamente per forzare sulla GPU 0
)
